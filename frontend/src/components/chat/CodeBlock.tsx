...
function DownloadButton({
  ...
  const handleDownload = useCallback(() => {
    const ext = language && languageExtensions[language] ? languageExtensions[language] : "txt"
...
export const CodeBlock = memo(function CodeBlock({
  ...
  return (
    <div
      className={cn("group/code my-4 w-full overflow-hidden rounded-xl border border-border", className)}
      ...
      {hasControls && (
        <div
          className={cn(
            ...
            showControlsOnHover && "h-0 overflow-hidden border-t-0 py-0 opacity-0 transition-all group-hover/code:h-auto group-hover/code:border-t group-hover/code:py-1.5 group-hover/code:opacity-100"
          )}
        >
          {showLanguage && language && (
            <span className="mr-auto font-mono text-xs lowercase text-muted-foreground">{language}</span>
          )}
          {showDownload && (
