...
function extractTableData(tableElement: HTMLTableElement): { text: string; csv: string } {
  const rows: string[][] = []
...
export const TableBlock = memo(function TableBlock({
  ...
  const [tableData, setTableData] = useState<{ text: string; csv: string }>({ text: "", csv: "" })
  ...
  return (
    ...
    >
      <div ref={tableRef} className="overflow-x-auto [&>*]:my-0 [&_p]:my-0 [&_p]:py-0">
      ...
      {hasControls && (
        <div
          className={cn(
            ...
            showControlsOnHover && "h-0 overflow-hidden border-t-0 py-0 opacity-0 transition-all group-hover/table:h-auto group-hover/table:border-t group-hover/table:py-1.5 group-hover/table:opacity-100"
          )}
        >
          {showDownload && (
