...
function setApiKey(scope: ApiKeyScope, provider: LLMProvider, apiKey: string) {
  switch (scope.type) {
    ...
    case "team":
      return apiKeysApi.setTeamKey(scope.orgId, scope.teamId, { provider, api_key: apiKey })
  }
}
...
function setDefaultProvider(scope: ApiKeyScope, provider: LLMProvider) {
  switch (scope.type) {
    ...
    case "team":
      return apiKeysApi.setTeamDefaultProvider(scope.orgId, scope.teamId, { provider })
  }
}
...
export function DefaultProviderSelector({ scope, currentProvider, isLoading }: DefaultProviderSelectorProps) {
  const queryClient = useQueryClient()

  const updateMutation = useMutation({
    mutationFn: (provider: LLMProvider) => setDefaultProvider(scope, provider),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: getDefaultProviderQueryKey(scope) })
    },
  })
    ...
    <div className="flex items-center justify-between rounded-md bg-muted/30 px-3 py-2">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <div className="text-[10px] text-muted-foreground">{getLevelLabel()}</div>
        </div>
      </div>
      <div className="flex items-center gap-1">
        {isConfigured && (
          <Badge variant="secondary" className={`text-[10px] h-4 ${hasOwnKey ? "bg-emerald-500/15 text-emerald-600" : ""}`}>
            {hasOwnKey ? <Check className="size-2 mr-0.5" /> : level === "org" ? <Building2 className="size-2 mr-0.5" /> : null}
...
export function SetApiKeyDialog({ provider, scope, hasKey }: SetApiKeyDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant={hasKey ? "ghost" : "outline"} size="sm" className="h-6 text-[10px] px-2">
          {hasKey ? "Update" : "Set"}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base">{hasKey ? "Update" : "Set"} {PROVIDER_INFO[provider].name} Key</DialogTitle>
        </DialogHeader>
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }}>
          <div className="space-y-3 py-3">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                />
                <Button type="button" variant="ghost" size="icon" className="absolute right-0 top-0 h-8 w-8" onClick={() => setShowKey(!showKey)}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" size="sm" disabled={!apiKey || mutation.isPending}>
              {mutation.isPending && <Loader2 className="mr-1.5 size-3 animate-spin" />}
              Save
            </Button>
          </DialogFooter>
        </form>
...
export function DeleteApiKeyButton({ provider, scope }: DeleteApiKeyButtonProps) {
  const queryClient = useQueryClient()
  ...
  const fallbackMessage = scope.type === "team"
