...
async function registerWithInvitationApi(data: RegisterWithInvitationData): Promise<User> {
 ...
 */
export function useRefreshToken(): UseMutationResult<Token | null, Error, void> {
  const queryClient = useQueryClient()
...
export function useRegisterWithInvitation() {
  ...
  return useMutation({
    mutationFn: async (data: RegisterWithInvitationData & { email: string }) => {
