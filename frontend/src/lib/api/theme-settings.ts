...
export const themeSettingsApi = {
  ...
  updateOrgSettings: (orgId: string, settings: OrganizationThemeSettingsUpdate) =>
  ...
  updateTeamSettings: (orgId: string, teamId: string, settings: TeamThemeSettingsUpdate) =>
  ...
  getPredefinedThemes: () =>
    apiClient.get<Record<string, ThemeColors>>("/v1/theme-settings/predefined-themes", {
      headers: getAuthHeader(),
    }),
}
