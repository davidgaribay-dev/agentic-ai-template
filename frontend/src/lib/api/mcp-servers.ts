...
export const mcpServersApi = {
  ...
  listOrgServers: (orgId: string) =>
    apiClient.get<MCPServersPublic>(`/v1/organizations/${orgId}/mcp-servers`, {
  ...
  createOrgServer: (orgId: string, data: MCPServerCreate) =>
    apiClient.post<MCPServer>(`/v1/organizations/${orgId}/mcp-servers`, data, {
      headers: getAuthHeader(),
    }),
  ...
  getUserServer: (serverId: string) =>
    apiClient.get<MCPServer>(`/v1/mcp-servers/me/${serverId}`, {
      headers: getAuthHeader(),
    }),
  ...
  updateUserServer: (serverId: string, data: MCPServerUpdate) =>
    apiClient.patch<MCPServer>(`/v1/mcp-servers/me/${serverId}`, data, {
      headers: getAuthHeader(),
    }),

  /** Delete a user's personal MCP server */
  deleteUserServer: (serverId: string) =>
    apiClient.delete<void>(`/v1/mcp-servers/me/${serverId}`, {
      headers: getAuthHeader(),
    }),
