...
def get_safe_extension(filename: str) -> str:
    """Extract file extension if it's in the allowed list.

    Args:
        filename: Original filename from user upload

    Returns:
        Lowercase extension if allowed, empty string otherwise
    """
...
async def process_document_task(
    ...
    try:
        ...
        # The filename is generated by tempfile, not from user input
        try:
            file_content = get_document_content(s3_object_key)
            # Get extension from allowlist only - no user input in path
            suffix = get_safe_extension(doc.filename)
            # Create temp file with secure, system-generated name
