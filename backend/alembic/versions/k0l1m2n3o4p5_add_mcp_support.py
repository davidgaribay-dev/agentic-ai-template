...
def upgrade() -> None:
    ...
    op.add_column(
        'organization_settings',
        sa.Column('mcp_allow_custom_servers', sa.Boolean(), nullable=False, server_default=sa.text("true"))
    )
    op.add_column(
        'organization_settings',
        sa.Column('mcp_max_servers_per_team', sa.Integer(), nullable=False, server_default=sa.text("10"))
    )
    op.add_column(
        'organization_settings',
        sa.Column('mcp_max_servers_per_user', sa.Integer(), nullable=False, server_default=sa.text("5"))
    )
    # Remove server defaults after adding columns
    op.alter_column('organization_settings', 'mcp_enabled', server_default=None)
    op.alter_column('organization_settings', 'mcp_allow_custom_servers', server_default=None)
    op.alter_column('organization_settings', 'mcp_max_servers_per_team', server_default=None)
    op.alter_column('organization_settings', 'mcp_max_servers_per_user', server_default=None)
    ...
    op.create_table(
        ...
        sa.Column('tool_prefix', sa.Boolean(), nullable=False, server_default=sa.text("true")),
