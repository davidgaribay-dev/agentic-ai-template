...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "team_role", sqlmodel.sql.sqltypes.AutoString(length=20), nullable=True
        ),
        sa.Column(
            "status",
            sqlmodel.sql.sqltypes.AutoString(length=20),
    ...
    op.add_column(
        "conversation", sa.Column("organization_id", sa.Uuid(), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_conversation_created_by_id", "conversation", type_="foreignkey"
    )
    op.drop_constraint("fk_conversation_team_id", "conversation", type_="foreignkey")
    op.drop_constraint(
        "fk_conversation_organization_id", "conversation", type_="foreignkey"
    )
